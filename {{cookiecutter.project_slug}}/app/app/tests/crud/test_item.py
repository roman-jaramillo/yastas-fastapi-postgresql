...
async def test_delete_item(db: AsyncSession) -> None:
    ...
    user = await create_random_user(db)
    item = await crud.item.create_with_owner(db=db, obj_in=item_in, owner_id=user.id)
    item2 = await crud.item.remove(db=db, id=int(item.id))
    item3 = await crud.item.get(db=db, id=item.id)
    assert item3 is None
    assert item2.id == item.id  # type: ignore
    assert item2.title == title  # type: ignore
    assert item2.description == description  # type: ignore
    assert item2.owner_id == user.id  # type: ignore
